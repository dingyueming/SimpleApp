...
namespace Simple.Repository
{
    public class LastLocatedRepository : BaseRepository<LastLocatedEntity>, ILastLocatedRepository
    {
        ...
        {
            var sql = "SELECT V.mac,N.* FROM NEWTRACK_LASTLOCATED N JOIN CARS V ON V.carid=N.CARID JOIN AUTH_LIMITS A ON N.CARID= A.CARID WHERE N.LOCATE =1 AND A.USERID=:USERID";
